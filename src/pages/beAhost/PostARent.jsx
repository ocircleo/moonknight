...
const PostARent = () => {
    ...
    const submit = e => {
        ...
        fetch("https://moonknight-backend.vercel.app/host/postHouse", {
            ...
            .then(res => res.json())
            .then(data => {
                toast(`house info uploaded`)
                if (data.insertedId) {
                    uploadImg(data.insertedId, images.files[0]).then(data => toast(`${images.files[0].name} uploaded`)).catch(error => console.log(error))
                    uploadImg(data.insertedId, images.files[1]).then(data => toast(`${images.files[1].name} uploaded`)).catch(error => console.log(error))
                    uploadImg(data.insertedId, images.files[2]).then(data => toast(`${images.files[2].name} uploaded`)).catch(error => console.log(error))
                    uploadImg(data.insertedId, images.files[3]).then(data => toast(`${images.files[3].name} uploaded`, form.reset())).catch(error => console.log(error))
