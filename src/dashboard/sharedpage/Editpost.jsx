...
const Editpost = () => {
    ...
    useEffect(() => {
        ...
        if (searchData.city) {
            fetch(`https://moonknight-backend.vercel.app/user/productSearch?city=${searchData.city}&region=&price=0&skip=0`).then(res => res.json()).then(d => { setData(d);  })
        } else {
            setData(initialData)
        }
    ...
    const search = (e) => {
        ...
        fetch(`https://moonknight-backend.vercel.app/user/productSearch?city=${city}&region=${region}&price=0&skip=0`).then(res => res.json()).then(d => { setData(d); setSpinner(false) })
    }
    const sort = (e) => {
        const region = document.getElementById('region').value;
        const city = document.getElementById('city').value;
        fetch(`https://moonknight-backend.vercel.app/user/productSearch?city=${city}&region=${region}&price=${e}&skip=0`).then(res => res.json()).then(d => { setData(d) })
    }

    return (
        <div className='w-full px-2 mx-auto mt-6'>
            ...
            {spinner ? (
                ...
                (
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4 md:px-10 p-5'>
                        <p className={`${data.length == 0 ? 'block' : 'hidden'} text-xl text-center col-span-3`}> Noting to show</p>
                        {
                            data.map(ele => <Post key={ele._id} ele={ele}></Post>)
                        }
